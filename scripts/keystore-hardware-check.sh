...
snapshot() {
  ...
  # the store master.
  appfiles "cd $KEYS_DIR || exit 0; for f in *; do [ -e \"\$f\" ] || continue; printf \"%s %s %s\n\" \"\$f\" \"\$(sha256sum \"\$f\" | cut -d\" \" -f1)\" \"\$(head -c 5 \"\$f\" | od -An -c | tr -d \" \n\")\"; done"
}
...
if [ "$PHASE" != "--after" ]; then
  ...
  if ! printf '%s\n' "$packages" | grep -x "package:$PKG" >/dev/null; then
...
if a logcat -d | grep -i "hardware keystore was never registered" >/dev/null; then
