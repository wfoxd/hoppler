...
fn a_chat_to_someone_out_of_range_is_queued_rather_than_refused() {
    ...
    // at the next direct encounter". This used to answer `Err`, and a phone
    // showed the consequence — writing to a paired peer who had gone gave
    // "Error: no session with …" while the message sat safely in the thread.
    // The app contradicted the requirement to the one person who could see
    // both halves.
    let _g = LOCK.lock().unwrap_or_else(|p| p.into_inner());
    let _h = fresh();
    ...
    assert!(thread.is_some(), "no thread was created");
    let msgs = thread_messages(thread.unwrap()).unwrap();
    assert_eq!(msgs.len(), 1);
    assert!(msgs[0].outgoing && msgs[0].text == "hi");

    // And it is *queued*, not counted as away — which is the difference
    // between this and a send that worked, and what the next reunion acts on.
